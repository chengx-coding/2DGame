...
ImageFactory::~ImageFactory() {
    ...
    std::map<std::string, SDL_Surface*>::const_iterator siter = surfaces.begin();
    while (siter != surfaces.end()) {
        SDL_FreeSurface(siter->second);
        siter++;
    }
    std::map<std::string, SDL_Texture*>::const_iterator titer = textures.begin();
    while (titer != textures.end()) {
        SDL_DestroyTexture(titer->second);
        titer++;
    }
